        ...
        <form method="POST" action="{{ route("admin.ads.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('gate_name') ? 'is-invalid' : '' }}" name="gate_name_id" id="gate_name_id" required>
                    @foreach($gate_names as $id => $entry)
                        <option value="{{ $id }}" {{ old('gate_name_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('image_select') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="image_select" id="image_select" value="1" {{ old('image_select', 0) == 1 || old('image_select') === null ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('news') ? 'is-invalid' : '' }}" type="text" name="news" id="news" value="{{ old('news', '') }}">
                @if($errors->has('news'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('time_entry') ? 'is-invalid' : '' }}" type="number" name="time_entry" id="time_entry" value="{{ old('time_entry', '') }}" step="1" required>
